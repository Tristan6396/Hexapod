...
//  C   REAR  D
//
// --I'd like to implement the inverse kinematics into this arduino code
//    rather than relying on outside generated arrays but I'm not sure how
//    to go about this. Doing this would allow for more complex motions.
//    Adding a gyroscope would even allow for adjusting to steep inclines.
// --Should the hexapod have a 'front'? Turning the front toward the direction
//    of movement just results in more opportunities to catch on things. Maybe
//    the hexapod should be able strafe along the ground instead.
// --One possibility is a tilt sensor attached to the chassis of the hexapod
//    maybe paired with a sensor on the tip of each foot which can sense
//    pressure or some other indicator that it is touching the ground, allowing
//    many complex tasks like:
...
//   -Tripod: classic hexapod gait. Three legs swing forward while the other three        \/
//     support and move the body forward. Statically stable, reliable, baseline           | https://hexapodrobots.weebly.com/types-of-robot-gait.html
//     speed. Most generally useful.                                                      |
//   -Quadruped: Two (probably) symmetric legs swing forward while the other four         |
//     support and move the body forward. Very stable but of course slower than tripod.   |
//     Likely would be useful to have for feeling out the terrain with the two front      |
//     feet.                                                                              |
//   -One-by-one: Extremely slow but stable. One leg swings forward at a time as          |
//     the body inches forward with the other five legs. Could be useful for very         |
//     difficult terrain but is probably overkill.                                        ^
//   -One-by-one but better: This is ripple (and wave) gait. These gaits move             \/
//     multiple legs at a time, but each starts moving offset from the others and         | https://hackaday.io/project/21904-hexapod-modelling-path-planning-and-control/log/62326-3-fundamentals-of-hexapod-robot
//     the speed can be changed to ensure a good balance of speed and stability. Ripple   |
//     gait also has a significant cool factor and is very spidery.                       ^
//   -All: Effectively belly-flopping. All six legs swing forward at once and then
//     all six legs move the body forward. Maybe useful for ice or something. Probably
//     not. Definitely funny anyways. If the servos are fast enough, this may be used
//     to jump, though it definitely would not be fast enough to catch itself if it's
//     jumping forward and would just slam.
//   -Bipod: 'Faster than nature.' Only two legs are on the ground at once. This is       \/
//     very fast, but is NOT statically stable. As long as the hexapod is on flat         | https://youtu.be/QZt3eJzHLSU?si=h863vg1OPpXfI5gd
//     ground, it should be able to run using this gait relying on dynamic stability.     ^ https://youtu.be/DKsyJYkdZ_o?si=HSgxqJErNcM0EqTi
// --I've also been looking at this guys hexapod project on youtube. His stuff isn't
//    perfectly applicable to our situation (mostly because his hexapod is not
//    autonomous) but he's uploaded all his code and talks a bit about the process
//    on his channel. I haven't taken the time needed to decipher enough to pull the
//    necessary parts into this project, but maybe something in his code will save us
//    a lot of time.
//        -> https://github.com/Ryan-Mirch/Aecerts_Hexapod_V1
//        -> https://www.youtube.com/@AecertRobotics
//
// --Should really upload this to a public github repository
//
